...
import { getLatestHackathon } from "../modules/services/hackathon.service";
import { updateProjectCodeInsights } from "../modules/services/project.service";
import { repoTypeConfig, RepoType } from "../config/repoTypeConfig";
...
const invokeCodeAgent = async (repoLink: string, projectId: string): Promise<void> => {
  ...
  const repoType: RepoType = "monorepo"; // 👈 you can pass this dynamically
  const { includePaths, ignorePaths } = repoTypeConfig[repoType];
  ...
  try {
    const loader = new GithubRepoLoader(repoLink, {
      ...
      maxConcurrency: 5,
      ignorePaths,
      // includePaths, // Removed as it is not a valid property
    });
